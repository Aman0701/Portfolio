...
import { PersonalNewsLetterSchema } from "../Schema/personalNewsletterSchema.js";

const PersonalModel = new mongoose.model('PersonalNewsLetter', PersonalNewsLetterSchema);

export default class PersonalNewsLetterRepository {

    static async addUser(userid) {
        
        try {
            const user = await PersonalModel.findOne({ userId: userid });
            ...
            if (user) {
                ...
                const newUser = new PersonalModel({ userId: userid, startDate: formattedDate, endDate: formattedFutureDate });
                await newUser.save();
                return newUser;
            }
        } catch (err) {
    ...
    static async getUser() {
        try {
            const users = await PersonalModel.find();
            return users;
        } catch (err) {
            console.log(err);
    ...
    static async deleteUser(userid){
        try{
            const user = await PersonalModel.deleteOne({_id:userid});
            return user;
        }catch(err){
            console.log(err);
